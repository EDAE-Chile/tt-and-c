/* Software from the nanopb project (https://github.com/nanopb/nanopb/)
...
struct pb_field_iter_s {
  ...
  const pb_msgdesc_t *submsg_desc; /* For submessage fields, pointer to field descriptor
                                      for the submessage. */
};
...
#define PB_FIELDINFO_AUTO2(width, tag, type, data_offset, data_size, size_offset, \
                           array_size)                                            \
  PB_FIELDINFO_AUTO3(width, tag, type, data_offset, data_size, size_offset, array_size)

#define PB_FIELDINFO_AUTO3(width, tag, type, data_offset, data_size, size_offset, \
                           array_size)                                            \
...
#define PB_FIELDINFO_ASSERT_AUTO2(width, tag, type, data_offset, data_size,        \
  ...
  PB_FIELDINFO_ASSERT_AUTO3(width, tag, type, data_offset, data_size, size_offset, \
                            array_size)
 ...
 * 1 word:  [2-bit len] [6-bit tag] [8-bit type] [8-bit data_offset] [4-bit size_offset]
 * [4-bit data_size]
 *
 ...
 * problem in the automatic logic - this may be worth reporting as a bug. As a
 * workaround, you can increase the descriptor width by defining PB_FIELDINFO_WIDTH or
 * by setting descriptorsize option in .options file.
 */
 ...
 * Note that the checks are against 31 bits to avoid compiler warnings about shift wider
 * than type in the test. I expect that there is no reasonable use for >2GB messages
 * with nanopb anyway.
 */
#define PB_FIELDINFO_ASSERT_4(tag, type, data_offset, data_size, size_offset, \
                              array_size)                                     \
